...
{
	int i,j,z,t,**gfun,random,sum,loop,tloop;
		...
		for(t=0;t<cent;t++)
		{
			...
			for(i=0;i<L;i++)
			{
				for(j=0;j<k;j++)
				{
					for(z=0;z<length;z++)
					{
						sum+=htable[i][j][z] * clist[i].centro->key1[z];
					}
					if(sum>=0)
						token[j]='1';
					else 
				...
				binarynum=atoll(token);
				decimalnum=turnintodecimal(binarynum);//thesi sto hastable pou tha mpei
				tloop=searchcosine(hasht[i][decimalnum].lista,clist[i].centro->key1,clist[i].centro->id,&dilist[i],length,radius,L);//search lsh
				if(tloop!=0)
					loop++;
			}
		}
